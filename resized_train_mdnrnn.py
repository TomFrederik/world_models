...
def get_act_batches(data, batch_size):
    ''' 
    params:
    data: np array of shape (N_rollouts, N_steps, 3)
    batch_size: int
...
def train(config):
    ...
    id_str = 'resized_mdnrnn_epochs_{}/lr_{}/temp_{}'.format(epochs, learning_rate, config.temp)
    ...
    # get data
    print('Loading data..')
    ac_dir = '/home/tom/disk_1/world_models_data/all_rollouts_resized/'
    enc_dir = '/home/tom/disk_1/world_models_data/enc_data_resized/'
    ...
    for obs_file, ac_file in zip(enc_files,ac_files):
        ...
        ac_data = np.load(ac_file, allow_pickle = True)['acs']
